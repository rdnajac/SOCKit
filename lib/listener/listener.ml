...
let setup_server_socket port =
  ...
  bind sock socket_addr;
  (* Bind the socket to the address *)
  listen sock 10;
  (* Listen on the socket, with a max of 10 pending connections *)
  sock
...
let handle_client client_sock =
  ...
  try
    while true do
      ...
      (* Flush stdout to ensure the data is printed immediately *)
    done
  with
  ...
  | client_sock, client_addr ->
      ...
      | ADDR_INET (addr, port) ->
          Printf.printf "Accepted connection from %s:%d\n"
            ...
            port
      | _ -> ());
      (* Handle non-INET addresses if necessary *)
