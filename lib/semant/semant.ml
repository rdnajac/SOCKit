...
let check (globals, functions) =
  let check_binds (kind : string) (binds : bind list) =
    List.fold_left (fun acc (ty, name) ->
  ...
  (* Ensure no duplicate global variables *)
  let _ = check_binds "global" globals in

  (* Built-in function declarations *)
  let built_in_decls = [
    { rtyp = Void; fname = "print"; formals = [(Int, "x")]; locals = []; body = [] }
  ] in
  let built_in_names = List.map (fun fd -> fd.fname) built_in_decls in

  (* Add function to symbol table, ensuring no duplicates or redefinitions of built-ins *)
  let add_func map fd =
    if List.mem fd.fname built_in_names then
      raise (Failure ("function " ^ fd.fname ^ " may not be defined as it is a built-in"))
    else if StringMap.mem fd.fname map then
      raise (Failure ("duplicate function definition: " ^ fd.fname))
    else
      StringMap.add fd.fname fd map
  in

  (* Fold over functions to create a complete symbol table *)
  let function_decls = List.fold_left add_func StringMap.empty (built_in_decls @ functions) in

  (* Function to find a function declaration by name *)
  ...
  let check_function func =
    (* Verify no duplicate parameters or locals, and no void types *)
